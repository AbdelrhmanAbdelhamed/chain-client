...
describe('new ChainClientProviderEngine()', function() {
  it('should work', function() {
    assert.doesNotThrow(function() {
      fixtureChainClient.engine.start();
    });
  });
});
...
describe('.getBalance()', function() {
  it('should work', async function() {
    const balance = await fixtureChainClient.getBalance();
    assert.notStrictEqual(balance, '0');
  });
});

describe('.getAccount()', function() {
  assert.strictEqual(fixtureChainClient.getAccount(), '0x627306090abab3a6e1400e9345bc60c78a8bef57');
});

describe('.sendTransaction()', function() {
  it('should signing and broadcasting a transaction of 1 ether from the first account to the second', async function() {
    const oldBalance = await fixtureChainClient.getBalance();
    const receipt = await fixtureChainClient.sendTransaction({
      to: '0xf17f52151EbEF6C7334FAD080c5704D77216b732', // second address/account
      value: '1' // 1 ether
    });
    const newBalance = await fixtureChainClient.getBalance();
    assert.ok(receipt);
